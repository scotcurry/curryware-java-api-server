...
public class DatabaseConfigValidator {
    ...
    private void validateEnvVariables() {
        if (database == null) {
            logger.warn("POSTGRES_DATABASE environment variable is not set");
        }
        if (password == null) {
            logger.warn("POSTGRES_PASSWORD environment variable is not set");
        }
        if (port == null) {
            logger.warn("POSTGRES_PORT environment variable is not set");
        }
        if (server == null) {
            logger.warn("POSTGRES_SERVER environment variable is not set");
        }
        if (username == null) {
            logger.warn("POSTGRES_USERNAME environment variable is not set");
        }
    }
